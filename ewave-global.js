var eWave = {
    ...
    'Qrcode': {
      'Init': function () {
        $('.ewave-qrcode').each(function () {
          var $that = $(this);
          $that.qrcode({
            text: $that.attr("data-url") || location.href, //11
            render: "canvas", //22  
            width: $that.attr("data-width") || 120, //33   
            height: $that.attr("data-height") || 120, //44  
            typeNumber: -1, //55
            correctLevel: 0, //66    
            background: "#ffffff", //77    
            foreground: "#000000" //88   
          });
        });
      }
    },
        ...
        if ($that.attr("data-id")) {
          if (!$that.hasClass("active")) {
            MAC.Ajax(maccms.path + '/index.php/ajax/digg.html?mid=' + $that.attr("data-mid") + '&id=' + $that.attr("data-id") + '&type=' + $that.attr("data-type"), 'get', 'json', '', function (r) {
              ...
              eWave.Layer.Error('v.v，，...');
            });
          } else {
            eWave.Layer.Error('oko！');
          }
        }
      });
    }
    ...
    'Form': function (obj, callback) {
        ...
        $that.find("input,textarea,select").each(function () {
          if ($(this).val() == '' && !$(this).hasClass("empty-allowed")) {
            ...
            if ($(this).attr("data-tip")) {
              ...
              eWave.Layer.Error(" " + $(this).attr("data-label"));
              return false;
            } else if ($(this).attr("placeholder")) {
              eWave.Layer.Error($(this).attr("placeholder"));
              return false;
            } else {
              eWave.Layer.Error(" @.@" + $(this).attr("name"));
              return false;
            }
          }
        });
      ...
      if ($that.hasClass("ewave-send-code")) {
        ...
        if (ac == 'email') {
          ...
          var ex = pattern.test(to);
          if (!ex) {
            eWave.Layer.Error('phone');
            return false;
          }
        } else {
          eWave.Layer.Error('ac ');
          return false;
        }
      }
      if ($that.hasClass("ewave-confirm")) {
        if (!$that.attr("data-tip")) {
          eWave.Layer.Error('m.m');
          return false;
        }
      }
      ...
      eWave.Check.Form($that, function (data) {
        eWave.Ajax(data.action, data.method, data.type, $that.parents("form").eq(0).serialize(), function (r) {
          ...
          if (r.code == 1) {
            ...
            var countdown = 60;
            eWave.CountDown.Run($that, countdown, function () {
              $that.val("disabled").removeAttr("disabled");
            }, function () {
              $that.prop("disabled", true).val("disabled(" + countdown + ")");
              countdown--;
            });
          } else {
            eWave.Layer.Error(r.msg);
          ...
          layer.closeAll('loading');
          eWave.Layer.Error('loading n1，loading n2');
        }, function () {
          layer.load();
          setTimeout(function () {
            layer.closeAll('loading');
  ...
  'Gbook': {
      ...
      $('body').on('click', '.ewave-gbook-report', function () {
        if ($(this).attr("data-id")) {
          if (eWave.Gbook.Login == 1 && eWave.User.IsLogin != 1) {
            ...
            eWave.Gbook.Report('  【' + $(this).attr("data-name") + '】   ', $(this).attr("data-id"));
          }
        }
      });
    },
    'Report': function (name, id) {
      MAC.Ajax(maccms.path + '/index.php/gbook/report.html?id=' + id + '&name=' + encodeURIComponent(name), 'post', 'json', '', function (r) {
        eWave.Layer.Html('Data error', r, function () {
          $(".ewave-gbook-content").focus();
          eWave.Image.Verify.Refresh($(".ewave-verify-img-report"));
        });
      }, function (XHR, textStatus, errorThrown) {
        eWave.Layer.Html('Data error', XHR.responseText, function () {
          $(".ewave-gbook-content").focus();
          eWave.Image.Verify.Refresh($(".ewave-verify-img-report"));
        });
      });
  ...
  'History': {
    ...
    'Init': function () {
      ...
      history_html += '<a class="ewave-history-text" href="javascript:;"><i class="fa fa-history"></i><span><span> 11</span><span> History</span></span></a>';
      history_html += '<div class="ewave-dropdown-box ewave-history-box">';
      if (jsondata.length > 0) {
        history_html += '<ul class="ewave-history-list">';
        history_html += '<li class="ewave-history-item ewave-history-head"><a><strong><i class="fa fa-history"></i>list </strong></a></li>';
        ...
        history_html += '<li class="ewave-history-item ewave-history-foot"><a class="ewave-history-clear" href="javascript:;"> clear ...</a></li>';
        history_html += '</ul>';
      } else {
        history_html += '<div class="ewave-history-empty"><i class="fa fa-minus-circle"></i>   😄    </div>';
      }
      history_html += '</div>';
      history_html += '</div>';
    ...
    'Clear': function () {
      ...
      $('.ewave-history-box').html('<div class="ewave-history-empty"><i class="fa fa-minus-circle"></i>&nbsp; #.#</div>');
    },
  },
  'Ulog': {
    'Init': function () {
        ...
        MAC.Ajax(maccms.path + '/index.php/user/ajax_ulog/?ac=list&limit=2000&type=2', 'get', 'json', '', function (r) {
          if (r.code == 1) {
            $.each(r['list'], function (index, row) {
              if (row.ulog_type == 2) {
                $(".ewave-ulog-get-" + row.data.id).addClass("active").html('<i class="fa fa-heart"></i>&nbsp;');
              }
            });
          } else {
          ...
          MAC.Ajax(maccms.path + '/index.php/user/ajax_ulog/?ac=set&mid=' + $that.attr("data-mid") + '&id=' + $that.attr("data-id") + '&type=' + $that.attr("data-type"), 'get', 'json', '', function (r) {
            if (r.code == 1) {
              ...
              if ($that.hasClass("ewave-ulog-get")) {
                $that.addClass("active").html('<i class="fa fa-heart"></i>&nbsp;');
              }
            } else {
              eWave.Layer.Error(r.msg);
            }
  ...
  'User': {
    ...
    'Init': function () {
      ...
      if (MAC.Cookie.Get('user_id') != undefined && MAC.Cookie.Get('user_id') != '') {
        ...
        if ($('.ewave-user').length > 0) {
          ...
          html += '<ul>';
          html += '<li><a href="' + maccms.path + '/index.php/user/favs.html"><i class="fa fa-user-o"></i><span>My save</span></a></li>';
          html += '<li><a href="' + maccms.path + '/index.php/user/plays.html"><i class="fa fa-id-card-o"></i><span>Play history</span></a></li>';

          html += '<li><a class="ewave-logout" href="javascript:;"><i class="fa fa-sign-out fa-rotate-180"></i><span>sign out</span></a></li>';
          html += '</ul>';
          html += '</div>';
          html += '</div>';
        ...
        $(".ewave-user").html('<a class="ewave-login" href="javascript:;"><i class="fa fa-user-o"></i><span>Log in</span></a>');
      }
    },
    'CheckLogin': function () {
      if (eWave.User.IsLogin == 0) {
    ...
    'BuyPopedom': function (o) {
      var $that = $(o);
      layer.confirm('Are you sure to purchase this data access permission?？', function () {
        MAC.Ajax(maccms.path + '/index.php/user/ajax_buy_popedom.html?id=' + $that.attr("data-id") + '&mid=' + $that.attr("data-mid") + '&sid=' + $that.attr("data-sid") + '&nid=' + $that.attr("data-nid") + '&type=' + $that.attr("data-type"), 'get', 'json', '', function (r) {
          ...
          eWave.Layer.Error('Network Error');
        });
      });
    }
  },
    ...
    'Show': function ($page) {
      if ($(".ewave-comment").length > 0) {
        MAC.Ajax(maccms.path + '/index.php/comment/ajax.html?rid=' + $('.ewave-comment').attr('data-id') + '&mid=' + $('.ewave-comment').attr('data-mid') + '&page=' + $page, 'get', 'json', '', function (r) {
          ...
          $(".ewave-comment").html('<div class="text-center ewave-empty" onClick="eWave.Comment.Show(' + $page + ')"><i class="fa fa-warning"></i>&nbsp;Comment，...</div>');
        });
      }
    },
  },
